...
if (isset($_POST['update_account'])) {
    ...
    $password = $_POST['password'] ? password_hash($_POST['password'], PASSWORD_BCRYPT) : $account['password'];
    $email = $_POST['email'];
    $role = $_POST['role'];

    $query = "UPDATE users SET username = ?, password = ?, email = ?, role = ? WHERE account_id = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("ssssi", $username, $password, $email, $role, $accountId);
    $stmt->execute();
    //$stmt->close();
...
<header class="header">
    ...
    <div class="container">
        ...
        <form action="" method="post">
            ...
            <input type="text" name="username" placeholder="Username" value="<?php echo htmlspecialchars($account['username']); ?>" required>
            <input type="password" name="password" placeholder="Password (để trống nếu không thay đổi)">
            <input type="email" name="email" placeholder="Email" value="<?php echo htmlspecialchars($account['email']); ?>" required>
            <select name="role" required>
                <option value="admin" <?php if ($account['role'] == 'admin') echo 'selected'; ?>>Admin</option>
                <option value="manager" <?php if ($account['role'] == 'manager') echo 'selected'; ?>>Manager</option>
